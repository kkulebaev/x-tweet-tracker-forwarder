...
- В конце поста добавь отдельный блок с кратким авторским выводом (2–4 предложения).
- Блок должен быть визуально отделён (например пустой строкой, или небольшой подзаголовок/маркер).
- Не обязательно подписывать его как «Моё мнение» и не ограничивайся фиксированными вариантами подписи — форма свободная.
...
Вывод:
- Верни только готовый текст поста (заголовок + текст + блок с авторским выводом), без комментариев, без JSON.`;
